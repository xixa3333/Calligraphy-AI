...
本專案採用 **5-Fold 交叉驗證 (5-Fold Cross-Validation)** 進行評估，在總計 **84,022 筆** 訓練資料中展現了穩定性與泛化能力。

### 1. 統計結果 (95% 信賴區間)

透過 5 次獨立的訓練與驗證循環，模型在驗證集上的表現如下：

| 辨識任務 | 平均準確度 (Mean Accuracy) | 變異數 (Variance) | 95% 信賴區間 (95% CI) |
| --- | --- | --- | --- |
| **書法名家 (Author)** | **93.14%** | 0.1014 | [92.75%, 93.54%] |
| **書體風格 (Style)** | **94.80%** | 0.1341 | [94.34%, 95.25%] |

### 2. 最終測試報告 (獨立測試集)

我們選取了 5-Fold 中表現最優的模型權重，在完全未參與訓練的 **21,007 筆** 獨立測試集上進行最終測試：

| 辨識任務 | 準確度 (Accuracy) | 說明 |
| :--- | :--- | :--- |
| **書法名家 (Author)** | **93.51%** | 在 20 位不同朝代的書法家中進行分類 |
| **書體風格 (Style)** | **94.89%** | 能精確區分楷書、行書、草書等 |

---

## 本地開發指南 (For Developers)
