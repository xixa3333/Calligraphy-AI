...
考慮到現代使用者多以硬筆（原子筆、鋼筆）書寫，且市面上缺乏完整的硬筆書法標註資料集，本系統導入了**骨架細化前處理流程**：

* **排除粗細干擾**：透過 **Zhang-Suen 骨架細化演算法**，將所有輸入圖片（無論是厚重的毛筆字或纖細的硬筆字）統一歸一化為「1 像素寬」的中心骨架。
* **筆劃結構修復**：在細化前自動執行形態學膨脹 (Dilation)，修復掃描或拍攝過程中產生的筆劃斷裂，確保字體結構的連貫性。
...
### 1. 環境設定
```bash
git clone https://github.com/xixa3333/Calligraphy-AI
cd Calligraphy-AI
